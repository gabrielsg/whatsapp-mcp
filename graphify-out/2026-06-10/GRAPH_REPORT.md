...
## Summary
- 731 nodes · 1479 edges · 65 communities (33 shown, 32 thin omitted)
...
1. `handleMessage()` - 45 edges
2. `T` - 43 edges
3. `newTestMessageStore()` - 41 edges
4. `T` - 41 edges
5. `newTestClient()` - 31 edges
6. `MessageStore` - 29 edges
7. `testLogger()` - 28 edges
8. `main()` - 22 edges
9. `str` - 22 edges
10. `str` - 21 edges
...
- `_resolve_lid_to_phone` --references--> `whatsmeow_lid_map`  [EXTRACTED]
  whatsapp-mcp-server/whatsapp.py → whatsapp.db
...
Cohesion: 0.12
Nodes (28): Any, int, str, Test chat without last message time., Test chat without last message time., Tests for chat conversion functions., Test direct message chat conversion., Test group chat conversion. (+20 more)

### Community 1 - "Go SQLite Data Types"
Cohesion: 0.06
Nodes (54): BasicCallMeta, Disappearing-message (ephemeral) settings tracking, ContextInfo, DB, DisappearingMode, HistorySync, MediaType, MessageInfo (+46 more)

### Community 2 - "Docs & CI Architecture"
Cohesion: 0.11
Nodes (82): Client, JID, Logger, T, Time, Context, HandlerFunc, LIDStore (+74 more)
...
Cohesion: 0.09
Nodes (25): int, str, main(), normalize_tag(), read_pyproject_version(), read_server_json_versions(), bootstrap-sha, changelog-path (+17 more)

### Community 5 - "HTTP Auth & Routing"
Cohesion: 0.10
Nodes (37): downloadMedia(), extractDirectPathFromURL(), download_media(), get_bridge_status(), get_chat(), get_contact(), get_contact_chats(), get_direct_chat_by_contact() (+29 more)
...
Cohesion: 0.29
Nodes (8): Bearer-token + Host-header authentication, Go REST API bridge (localhost:8080), store/.bridge-token file, _bridge_headers, download_media (whatsapp.py), _read_bridge_token, send_file (whatsapp.py), send_message (whatsapp.py)

### Community 10 - "Chat Sync & Metadata"
Cohesion: 0.25
Nodes (15): buildAllowedHosts, buildAllowedHosts(), checkBearerToken(), hostAllowed(), loadOrCreateBridgeToken(), T, TestCheckBearerToken(), TestHostAllowed() (+7 more)
...
Cohesion: 0.15
Nodes (27): bool, _bridge_headers(), download_media(), format_message(), format_messages_list(), get_bridge_status(), get_chat(), get_sender_name() (+19 more)
...
Cohesion: 0.22
Nodes (9): TestResolveLIDToPhone, TestSenderAliases, Tests for WhatsApp MCP server functions., Tests for _sender_aliases — verify it calls the bridge and falls back to DB., Tests for _resolve_lid_to_phone — verify it calls the bridge and falls back., TestResolveLIDToPhone, TestSenderAliases, _sender_aliases() (+1 more)
...
Cohesion: 0.16
Nodes (12): Tests for message conversion functions., Test basic message to dict conversion., Test message from self shows 'Me' as sender., Test message from self shows 'Me' as sender., Test message with media type., Test message with media type., TestMessageConversion, get_last_interaction (+4 more)

### Community 24 - "MCP Query Utilities"
Cohesion: 0.21
Nodes (12): Tests for contact conversion functions., Test contact to dict conversion., Test contact without name., Test contact without name., TestContactConversion, Contact, contact_to_dict(), Any (+4 more)

### Community 25 - "Community 25"
Cohesion: 0.18
Nodes (13): LID Resolution Consolidation Implementation Plan, LID-to-phone JID resolution, /api/resolve, ResolveResponse, MessageStore.MigrateLegacyLIDChatsToPhoneJIDs, MessageStore.MigrateLegacyLIDSendersToPhones, store/whatsapp.db (whatsmeow session DB), whatsmeow_lid_map (+5 more)
...
Cohesion: 0.19
Nodes (13): get_contact_chats(), get_last_interaction(), get_message_context(), get_message_context, list_messages(), Message, MessageContext, int (+5 more)
...
## Knowledge Gaps
- **191 isolated node(s):** `release-type`, `include-v-in-tag`, `include-component-in-tag`, `changelog-path`, `extra-files` (+186 more)
...
- **Why does `downloadMedia()` connect `HTTP Auth & Routing` to `WhatsApp MCP Test Suite`, `Go SQLite Data Types`, `Docs & CI Architecture`, `Media Path Safety`, `List Chats Tests`, `LID Migration`?**
  _High betweenness centrality (0.134) - this node is a cross-community bridge._
- **Why does `handleMessage()` connect `Docs & CI Architecture` to `Go SQLite Data Types`, `HTTP Auth & Routing`, `Audio Conversion & REST Auth`?**
  _High betweenness centrality (0.075) - this node is a cross-community bridge._
- **Why does `newRESTMux()` connect `Docs & CI Architecture` to `Media Path Safety`, `Go SQLite Data Types`, `Chat Sync & Metadata`, `HTTP Auth & Routing`?**
